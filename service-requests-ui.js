(() => {
  ...
  async function loadAssignees(cid) {
    if (!window.CondoAccess?.can('team.manage', cid)) return [];
  ...
  function requestRow(x) {
    ...
    return `<tr class="clickable" onclick="openServiceRequest('${x.id}','${x.condominium_id}')"><td><strong>${esc(x.title)}</strong><div class="list-sub">${esc(x.category || 'Sem categoria')}</div></td><td>${esc(c?.name || '—')}</td><td>${esc(x.location_label || '—')}</td><td>${esc(priorityLabels[x.priority] || x.priority)}</td><td>${esc(statusLabels[x.status] || x.status)}</td><td>${x.assigned_to ? 'Com responsável' : '<span class="muted">Não atribuído</span>'}</td></tr>`;
  }

  async function callsPage(cid = null) {
    const c = cid ? condo(cid) : null;
    $('#app').innerHTML = shell(`${topbar('Chamados', c ? `Solicitações de ${esc(c.name)}.` : 'Solicitações de todos os condomínios.', 'Operação')}<article class="panel"><div class="panel-body"><div class="empty">Carregando chamados...</div></div></article>`, cid ? 'condo-calls' : 'calls', cid);
